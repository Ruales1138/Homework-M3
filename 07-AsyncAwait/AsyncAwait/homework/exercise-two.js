...
async function problemA () {
  ...
  // AsyncAwait version
  let promesas = await Promise.all(['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'])
  .map(poema => promisifiedReadFile(poema));
  promesas.map(e => blue(e))
  console.log('-- A. callback version done --')
}
...
async function problemB () {
  ...
  // AsyncAwait version
  let promesas = await Promise.all(filenames)
  .map(poema => promisifiedReadFile(poema));
  promesas.map(e => blue(e))
  console.log('-- A. callback version done --')
}
